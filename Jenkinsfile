pipeline {
    ...
    environment {
        ...
        DEPLOY_DIR = "/opt/tomcat/webapps/" // Change if your Tomcat is elsewhere
    ...
    stages {
        ...
        stage('Code Quality') {
            steps {
                ...
                // Optional: configure SonarQube in Jenkins first
        ...
        stage('Deploy') {
            steps {
                ...
                // Ensure Jenkins has write permissions to Tomcat webapps folder
                sh "cp ${WAR_FILE} ${DEPLOY_DIR}"
            }
        }
        ...
        stage('Verify Deployment') {
            steps {
                ...
                script {
                    try {
                        sh "curl -s ${TOMCAT_URL} | grep 'Welcome'"
